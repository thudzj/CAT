...
tf.app.flags.DEFINE_float('unbalance', 1., 'Unbalance ratio') #set 10 for the imbalanced experiments
tf.app.flags.DEFINE_integer('revgrad', 0, 'Use revgrad or not')

FLAGS = tf.app.flags.FLAGS
MAX_STEP=10000
NUM_CLASSES = 10 if FLAGS.unbalance == 1. else 2
...
TEST=data_map[FLAGS.target]('data/' + FLAGS.target,split='train',shuffle=False, select=select_target, unbalance=1./FLAGS.unbalance)
print(len(TEST.labels))
for i in range(10):
        print((TRAIN.labels == i).sum(), (VALID.labels == i).sum())
